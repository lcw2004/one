...
<section class="content">
  <div class="box">
    ...
    <div class="box-body">
      ...
      <TableNoData :data="templateList"></TableNoData>
    </div>
...
export default {
  ...
  data: () => {
    return {
      actions: {
        listTemplateType: {method: 'get', url: '/api/base/templateType'},
        listTemplate: {method: 'get', url: '/api/base/template'},
        deleteData: {method: 'delete', url: '/api/base/template/{templateId}'}
      },
      templateTypeList: [],
      templateList: [],
      selectCode: '',
  ...
  methods: {
    loadTemplateType () {
      this.resource.listTemplateType().then(function (response) {
    ...
    loadTemplate (code) {
      this.resource.listTemplate({templateTypeCode: code}).then(function (response) {
        let result = response.body
        if (result.ok) {
    ...
    deleteTemplate (templateId) {
      this.$confirm('确认删除模板吗？', () => {
        this.resource.deleteData({templateId: templateId}).then(function (response) {
          let result = response.body
          if (result.ok) {
