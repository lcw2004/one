...
<img src="${pageContext.request.contextPath}/validateCode" onclick="$('.${name}Refresh').click();" class="mid ${name}"/>
<a href="javascript:" onclick="$('.${name}').attr('src','${pageContext.request.contextPath}/validateCode?'+new Date().getTime());" class="mid ${name}Refresh">看不清</a>